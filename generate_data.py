#!/usr/bin/env python3
"""
Generate the complete data.js for FishID with 130+ Ohio & Lake Erie fish species.
Run: python3 generate_data.py > js/data.js
"""
...
###############################################################################
# HELPER: Format a fish dict as JavaScript object
###############################################################################
def js_fish(f):
    """Convert a fish dict to a JS object string."""
    features_str = ',\n      '.join(f"'{esc(feat)}'" for feat in f['features'])
    return f"""  {f['id']}: {{
    id: '{f['id']}', name: '{esc(f['name'])}', scientific: '{esc(f['scientific'])}',
    image: '{f['image']}',
    quizImage: '{f['quizImage']}',
    features: [
      {features_str}
    ],
    keyFeature: '{esc(f['keyFeature'])}',
    habitat: '{esc(f['habitat'])}',
    size: '{esc(f['size'])}',
    funFact: '{esc(f['funFact'])}',
    colors: {{ primary: '{f['colors']['primary']}', secondary: '{f['colors']['secondary']}', belly: '{f['colors']['belly']}', fin: '{f['colors']['fin']}' }}
  }}"""
...
# ===================== EXISTING 20 FISH (verbatim data) =====================
...
TRUE_FALSE = [
    ...
    # New statements for new species
...
def generate():
    out = []
    out.append('// ===== FISH SPECIES DATA =====')
    out.append('const FISH = {')
    ...
    # LESSONS
    out.append('// ===== LESSON DEFINITIONS =====')
    out.append('const LESSONS = [')
    for i, lesson in enumerate(LESSONS):
        fish_str = ', '.join(f"'{fid}'" for fid in lesson['fish'])
        unlocked_str = 'true' if lesson['unlocked'] else 'false'
        out.append(f"  {{")
        out.append(f"    id: {lesson['id']}, title: '{esc(lesson['title'])}', icon: '{lesson['icon']}', description: '{esc(lesson['description'])}',")
        out.append(f"    fish: [{fish_str}],")
        out.append(f"    unlocked: {unlocked_str}")
    ...
    # SIMILAR PAIRS
    out.append('// ===== SIMILAR FISH PAIRS (for spot-the-difference questions) =====')
    out.append('const SIMILAR_PAIRS = [')
    for i, pair in enumerate(SIMILAR_PAIRS):
        comma = ',' if i < len(SIMILAR_PAIRS) - 1 else ''
        out.append(f"  {{ fish1: '{pair['fish1']}', fish2: '{pair['fish2']}', hint: '{esc(pair['hint'])}' }}{comma}")
    out.append('];')
    out.append('')
    
    # TRUE/FALSE
    out.append('// ===== TRUE/FALSE STATEMENTS =====')
    out.append('const TRUE_FALSE = [')
    for i, tf in enumerate(TRUE_FALSE):
        answer_str = 'true' if tf['answer'] else 'false'
        comma = ',' if i < len(TRUE_FALSE) - 1 else ''
        out.append(f"  {{ statement: '{esc(tf['statement'])}', answer: {answer_str}, fishId: '{tf['fishId']}' }}{comma}")
    out.append('];')
    out.append('')
    
    # FEATURE OPTIONS
    out.append('// ===== FEATURE QUIZ OPTIONS =====')
    out.append('const FEATURE_OPTIONS = {')
    keys = list(FEATURE_OPTIONS.keys())
    for i, key in enumerate(keys):
        opt = FEATURE_OPTIONS[key]
        wrong_str = ', '.join(f"'{esc(w)}'" for w in opt['wrong'])
        comma = ',' if i < len(keys) - 1 else ''
        out.append(f"  {key}: {{ correct: '{esc(opt['correct'])}', wrong: [{wrong_str}] }}{comma}")
    out.append('};')
...
if __name__ == '__main__':
    content = generate()
    with open('/Users/joeystout/clawd/fishid/js/data.js', 'w') as f:
        f.write(content)
    print(f"Generated data.js with {len(FISH)} fish species across {len(LESSONS)} lessons")
    print(f"  Similar pairs: {len(SIMILAR_PAIRS)}")
    print(f"  True/False: {len(TRUE_FALSE)}")
    print(f"  Feature options: {len(FEATURE_OPTIONS)}")
