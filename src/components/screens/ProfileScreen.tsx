...
export default function ProfileScreen({ onNavigate }: ProfileScreenProps) {
  ...
  const [selectedFishId, setSelectedFishId] = React.useState<string | null>(null);

  const selectedFish = selectedFishId ? FISH[selectedFishId] : null;
  const selectedFishImage = selectedFish ? getRandomFishImage(selectedFish.image, selectedFish.id) : '';
          ...
          {[...FISH_IDS]
            ...
            return (
              <div
                key={id}
                onClick={() => {
                  if (isDiscovered) setSelectedFishId(id);
                }}
                className={`
                  relative aspect-[4/3] rounded-xl overflow-hidden border transition-all duration-300
                  ${
                    isDiscovered
                      ? 'bg-panel border-white/10 shadow-lg cursor-pointer hover:scale-[1.01]'
            ...
            >
              ...
              <div className="relative h-64 w-full bg-black/20 shrink-0">
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-panel via-panel/80 to-transparent pt-16 pb-4 px-5">
                  ...
                  <div className="text-lg italic text-white/70 font-serif">
                    {selectedFish.scientific}
                  </div>
                </div>
              </div>
              ...
              <div className="p-5 pt-4 flex-1 overflow-y-auto">
                <div className="space-y-6 text-sm">
                  <div className="bg-white/5 p-4 rounded-xl border border-white/5 mb-2">
                    ...
                    <div className="text-white/90 leading-relaxed">{selectedFish.habitat}</div>
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="space-y-3">
                      {selectedFish.features?.map((f: string, i: number) => (
                        <li
                          key={i}
                    ...
                    <div className="bg-gold/10 p-4 rounded-xl border border-gold/20">
                      <div className="flex gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-gold/90 italic leading-relaxed">
                            {selectedFish.funFact}
                          </span>
                        </div>
                      </div>
